...
class LoginScreen(Screen):
    ...
    BINDINGS = [
        Binding("ctrl+r", "register", "Register", priority=True),
        Binding("q", "quit", "Quit"),
    ]

    def compose(self) -> ComposeResult:
        with Container(id="login-box"):
            yield Static("  MooCash  ", id="login-title")
...
class MainScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="app-sidebar"):
            yield Static(" MooCash ", classes="sidebar-header")
        ...
        with Container(id="main-content"):
            yield Static(f"MooCash  |  {user_name}  |  {datetime.now().strftime('%Y-%m-%d %H:%M')}", id="status-bar")
